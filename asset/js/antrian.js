...
function ambil_antrian(kode){
	$.ajax({
		type: 'ajax',
		url: 'http://'+svr+'c_antrian/tambah/'+kode,
		dataType: 'json',
		beforeSend: function(){
			$(".loading").show();
		},
...
// {
	var tabel = $("#daftar_antrian").DataTable({
		...
		"ordering" : false,
		"ajax" : {
			"url" : "http://"+svr+"c_antrian/antrian/pengaduan",
			"dataSrc" : "",
		},
		"columns" : [
	...
	var b = setTimeout(function()
		{
			...
			tabel.ajax.url("http://"+svr+"c_antrian/antrian/pendaftaran").load();
			$("#layanan").text("PENDAFTARAN");
		},10000);
	var c = setTimeout(function()
		{
			
			tabel.ajax.url("http://"+svr+"c_antrian/antrian/pengambilan").load();
			$("#layanan").text("PENGAMBILAN PRODUK");
		},20000);
	var d = setTimeout(function()
		{
			
			tabel.ajax.url("http://"+svr+"c_antrian/antrian/ecourt").load();
			$("#layanan").text("E-COURT");
		},30000);
	var e = setTimeout(function()
		{
			
			tabel.ajax.url("http://"+svr+"c_antrian/antrian/kasir").load();
			$("#layanan").text("KASIR");
		},40000);
	var f = setTimeout(function()
		{
			
			tabel.ajax.url("http://"+svr+"c_antrian/antrian/POSBAKUM").load();
			$("#layanan").text("POSBAKUM");
		},50000);
	var g = setTimeout(function()
		{
			
			tabel.ajax.url("http://"+svr+"c_antrian/antrian/POS").load();
			$("#layanan").text("POS");
		},60000);
	var h = setTimeout(function()
		{
			
			tabel.ajax.url("http://"+svr+"c_antrian/antrian/bank").load();
			$("#layanan").text("BANK");
		},70000);
	var a = setTimeout(function()
		{
			
			tabel.ajax.url("http://"+svr+"c_antrian/antrian/pengaduan").load();
...
function panggil ()
{
	$.ajax({
		type: "GET",
		url: 'http://'+svr+'c_antrian/panggil_antrian',
		dataType: 'json',
		success: function(respon){
			if (respon.success==1)
			{
...
function hapus_panggil_antrian(id)
{
	$.ajax({
		type: "POST",
		url: 'http://'+svr+'c_antrian/hapus_panggil_antrian',
		data: {id: id},
		dataType: "JSON",
		success: function(respon)
		{
...
function drp(){
	...
	$.ajax({
		type: "ajax",
		url: 'http://'+svr+'c_antrian/drp/'+q+'er',
